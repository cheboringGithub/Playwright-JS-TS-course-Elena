...
💡 **Важно:** В отличие от [**`var`**]({{ site.baseurl }}/glossary/lesson-2-glossary#var-устаревшее-ключевое-слово){:data-glossary-term="var" data-glossary-definition="Устаревший способ объявления переменных"}, которое позволяет переобъявление, `let` и `const` более строгие и предотвращают случайные ошибки.
